...
export interface DatabaseAuthServiceInterface {
    decodeToken(token: string): Promise<{ success: boolean, details: object }>;
}
 ...
 */
abstract class AbstractAuthService implements DatabaseAuthServiceInterface {
    /**
     * Decode access token
     * @param {string} token A token to decode
     */
    abstract decodeToken(token: string): Promise<{ success: boolean, details: object }>
}
